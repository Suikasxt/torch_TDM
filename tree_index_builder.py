...
class TreeIndexBuilder:
    ...
    def build_by_category(self, input_filename, output_filename, tree_emb_path):
        class Item:
        ...
        print('before build')
        self.build(output_filename, ids, codes, tree_emb_path)
    ...
    def tree_init_by_kmeans(self, input_filename, output_filename, tree_emb_path, parall=1):
        t1 = time.time()
        ...
        if file_type == 'txt':
            ...
            with open(input_filename) as f:
                for line in f:
                    ...
                    data.append(vector)
                    self.item_emb[int(arr[0])] = vector
            self.ids = np.array(ids)
            self.data = np.array(data)
        elif file_type == 'npy':
            self.data = np.load(input_filename)
            self.ids = np.array(range(self.data.shape[0]))
        t2 = time.time()
        ...
        self.build(output_filename, self.ids, self.codes, tree_emb_path, data=self.data)
    ...
    def _minbatch(self, pcode, index, code):
        ...
        #print('Minbatch in {}'.format(pcode))
        tstart = time.time()
        while dq:
    ...
    def _cluster(self, index):
        ...
        if (data == data[0]).all():
            length=len(index)
            return index[:length//2], index[length//2:]
        ...
        '''if (len(index) <= 3 and (labels==0).all()):
            print(index)
            #print(data)
            print(labels)
            print(left_index)
    ...
    def save_node_info(self, codes, data, ids, id_offset, leaf_left,
        ...
        print(tree_emb_np)
        np.save(tree_emb_path, tree_emb_np)
    ...
    def build(self, output_filename, ids, codes, tree_emb_path, data=None, id_offset=None):
        # process id offset
        '''
        if not id_offset:
            max_id = 0
        ...
        self.save_node_info(codes, data, ids, id_offset, leaf_left, leaf_right, tree_emb_path)
        filter_set = set()
        max_level = 0
...
if __name__ == '__main__':
    ...
    parser.add_argument("--tree_emb_path", required=True, help="output filename")

    args = parser.parse_args()
    t1 = time.time()
    if args.mode == "by_category":
        builder = TreeIndexBuilder()
        builder.build_by_category(args.input, args.output, args.tree_emb_path)
    elif args.mode == "by_kmeans":
        builder = TreeIndexBuilder()
        builder.tree_init_by_kmeans(args.input, args.output, args.tree_emb_path, args.parallel)
    t2 = time.time()
    print('DONE!')
